...
<title>Application Registartion</title>
...
<!-- SsnNumber Validation -->
<script>
$(document).ready(function(){
	
	 $("#ssnNumber4").blur(function(){
	 var enterSsnNumber=$("#ssnNumber4").val();
	 $.ajax({
	 url :"ssnNumber",
	 data : {
		 ssnNumber4 : enterSsnNumber
	 },
	  success : function(result){
	  ...
	  if(result=='duplicate'){
	   ...
	   $("#ssnNumber4").empty;  
	  $("#ssnNumberError").html("**** Citizen not from washington ****");
	  $("#ssnNumber4").focus();
	  }
	  else{
	  $("#ssnNumberError").html("");
...
<body>
	<div id="page-wrapper">
		<div class="container">
			...
			<h2 style="color: red">${stateNameMessage}</h2>
			...
			<form:form name="myForm" class="form-horizontal" id="myForm"
				action="/insertARData" modelAttribute="arModel" method="POST">
